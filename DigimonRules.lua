...
--colors live in a child table, so pull them back with the row as "0,5"
local selectCard =
    [[
...
function DigimonRules.drawCards(amount)
    local cards = {}
    for data in database:nrows(
        string.format("%s and c.level==%d order by random() limit %d", selectCard, drawLevel, amount)
    ) do
...
--[[
    ...
    All three conditions are symmetric in a and b, so the fusion is commutative, and
