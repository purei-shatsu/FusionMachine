...
function Game:_drawCardsUntilFive(side)
    ...
    for _, cardModel in ipairs(GameMode.rules().drawCards(amount)) do
...
function Game:_isFusionBetter(resultA, materialsA, replacesA, resultB, materialsB, replacesB)
    ...
    --better stats win, by whatever the current game means by better
    local stats = GameMode.rules().compareStats(resultA, resultB)
    if stats ~= 0 then
        return stats > 0
    end
...
local function weakPowerOrder(a, b)
    return a:getModel():getPower() < b:getModel():getPower()
end

local function strongPowerOrder(a, b)
    return a:getModel():getPower() > b:getModel():getPower()
end
...
function Game:_makeAIAttacks()
    ...
    --order by power (weaker first)
    table.sort(attackers, weakPowerOrder)

    --for each attacker, attack the strongest card that is weaker than it
    for _, attacker in ipairs(attackers) do
        local attackerPower = attacker:getModel():getPower()
        ...
        --order by power (stronger first)
        table.sort(targets, strongPowerOrder)

        --attack first with power lower (or same power if has more cards)
        for _, target in ipairs(targets) do
            local targetPower = target:getModel():getPower()
            if attackerPower > targetPower or (hasMoreCards and attackerPower == targetPower) then
...
function Game:_attack(cardA, cardB)
    ...
    local defeated = {}
    local powerA = modelA:getPower()
    local powerB = modelB:getPower()
    if powerA <= powerB then
        defeated[#defeated + 1] = cardA
    end
    if powerB <= powerA then
